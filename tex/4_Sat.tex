...
after $\contradiction$ is generated in $I$, where all literals in the unsatisfied
...
explored with \emph{Unit Implication Points} (UIPs). A UIP is a vertex $u$ in
the implication graph, such that every path from a decision vertex $p$ to the
conflict vertex $\contradiction$ contains $u$. It represents an alternative
decision assignment at the current decision level that results in the same
conflict. The main motivation for identifying UIPs is to reduce the size of
learnt clauses. 
